...
const inputSpec = InputSpec.of({
  ...
  packageJson: Value.textarea({
    name: i18n('package.json (optional)'),
    description: i18n(
...
export const deployPlugin = sdk.Action.withInput(
  ...
  async ({ effects }) => ({
    name: i18n('Deploy Node.js Plugin'),
    description: i18n(
  ...
  async ({ input }) => {
    ...
    const scriptContent = script.startsWith('#!')
      ? script
      : `#!/usr/bin/env node\n${script}`
    ...
    return {
      ...
      result: {
        ...
        description: i18n(
          'Enter this as the Script value when registering the plugin in Cronicle',
        ),
        value: scriptPath,
        masked: false,
        copyable: true,
